...
| Products management | React Hook Form, Zod, Axios service, shadcn-style Card/Table/Badge, Sonner toast | `src/app/admin/products`, `src/modules/products`, `src/services/product.service.ts`, `src/services/upload.service.ts`, `md/product-module.md` | Quan ly san pham theo product-module: slug, SKU unique, status `DRAFT/ACTIVE/INACTIVE/OUT_OF_STOCK`, filter category/brand/origin/tags/rating/inStock, sort `newest/price_asc/price_desc/best_selling/rating`, variants, images, upload product images; UI map ca `id` va Mongo `_id` de gui dung category/brand/product id cho API |
...
| Inventories management | React Hook Form, Zod, Axios service, shadcn-style Card/Table/Badge, Sonner toast | `src/app/admin/inventories/page.tsx`, `src/modules/inventories`, `src/services/inventory.service.ts` | Quan ly ton kho qua `GET/PATCH /admin/inventories`, `POST /admin/inventories/import`, `POST /admin/inventories/adjust`; filter store, search product/SKU/barcode, tinh `availableQuantity = quantity - reservedQuantity`, canh bao `LOW_STOCK <= 5`, movement history neu backend co API |
